...
# Configuración de Observabilidad (Logs Granulares)
        ...
        try:
            async with session.request(method, url, params=params, json=json_body, timeout=15) as resp:
                if resp.status in [200, 201]: return await resp.json()
                    ...
                    wait = 1.5 * (attempt + 1)
                    logger.warning(f"Rate Limit {url}. Reintentando en {wait}s...")
            ...
            logger.error(f"Error Conexión ({attempt+1}/{retries}): {e}")
            await asyncio.sleep(1)
    return None

# --- LÓGICA DE GAMEPOINTS & BÓVEDA ---
async def crear_cupon_jumpseller(session, codigo: str, descuento: int, email: str):
    logger.info(f"Generando cupón {codigo} (${descuento}) para {email}")
    ...
    payload = {
        "promotion": {
            ...
            "status": "active",
            "usage_limit": 1, # Integridad: 1 uso total
...
async def fetch_scryfall_metadata(ids: List[str]) -> Dict[str, Any]:
    ...
    for uid in unique_ids:
        if uid in SCRYFALL_CACHE and (now - SCRYFALL_CACHE[uid][1] < CACHE_TTL): result[uid] = SCRYFALL_CACHE[uid][0]
...
# --- LÓGICA DE BUYLIST (PROCESAMIENTO CSV) ---
async def procesar_csv_logic(content: bytes, internal_mode: bool) -> List[Dict]:
    start_time = time.time()
    logger.info(f"Iniciando Buylist. Modo Interno: {internal_mode}")
    ...
    def parse_csv():
        try:
            ...
            logger.error(f"Error Parser CSV: {e}")
            return None

    loop = asyncio.get_event_loop()
    df = await loop.run_in_executor(None, parse_csv)
    if df is None: return {"error": "CSV con formato inválido"}
    ...
    logger.info(f"Buylist Finalizada: {len(final_data)} items procesados en {time.time()-start_time:.2f}s")
    return final_data

def enviar_correo_dual(cli, items, clp, gc, csv_content, fname):
    if not settings.SMTP_EMAIL: return
    rows = "".join([f"<tr><td>{i['quantity']}</td><td>{i['name']} <small>({i['set_code']})</small></td><td align='right'>${i.get('price_total',0):,}</td></tr>" for i in items])
    terms = "<div style='background:#eee;padding:10px;margin-top:15px;font-size:0.8em'><strong>Términos:</strong> Precios sujetos a revisión. Validez 24hrs.</div>"
    html = f"<h2>Buylist: {cli.get('nombre')}</h2><p>RUT: {cli.get('rut')} | Pago: {cli.get('metodo_pago')}</p><p><strong>Cash: {clp} | QP: {gc}</strong></p><table>{rows}</table>{terms}"
    try:
        ...
        msg2 = MIMEMultipart(); msg2['Subject'] = "Recibido GameQuest"; msg2['From'] = settings.SMTP_EMAIL; msg2['To'] = cli.get('email')
        msg2.attach(MIMEText(html.replace("Buylist:", "Recibido:"), 'html'))

    ...
    async with aiohttp.ClientSession() as session:
        while True:
            ...
            for d in data:
                c = d.get('customer', {})
                if c.get('email'):
                    customers.append({"email": c.get('email'), "name": f"{c.get('name','')} {c.get('surname','')}".strip()})
...
async def procesar_csv_logic(content: bytes, internal_mode: bool) -> List[Dict]:
    def parse():
        try:
            ...
            map_cols = {"Name": "name", "Set code": "set_code", "Foil": "foil", "Quantity": "quantity", "Purchase price": "purchase_price", "Purchase Price": "purchase_price", "Scryfall ID": "scryfall_id", "Meta Name": "meta_name"}

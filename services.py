...
async def procesar_csv_logic(content: bytes, internal_mode: bool) -> List[Dict]:
    ...
    logger.info(f"Procesando CSV. Internal: {internal_mode}")
    
    def parse_csv():
        try:
            df = pd.read_csv(io.BytesIO(content))
            df.columns = [c.strip() for c in df.columns]
            df.rename(columns={"Name": "name", "Set code": "set_code", "Foil": "foil", "Quantity": "quantity", "Purchase price": "purchase_price", "Scryfall ID": "scryfall_id", "Meta Name": "meta_name"}, inplace=True)
            ...
            # Agrupación pre-cálculo
            df["_v"] = df["quantity"] * df["purchase_price"]
            df = df.groupby(["name", "set_code", "foil", "scryfall_id", "meta_name"], as_index=False).agg({"quantity":"sum", "_v":"sum"})
    ...
    if df is None: return {"error": "CSV corrupto o formato inválido"}
    ...
    async with aiohttp.ClientSession() as sess:
        stock_res = await asyncio.gather(*[get_jumpseller_stock(sess, n) for n in df["clean_name"].unique()])
        stock_map = dict(zip(df["clean_name"].unique(), stock_res))

    # Lógica Iterativa para Observabilidad Fila a Fila
    def logic_core(df):
        results = []
    ...
    final_data = await loop.run_in_executor(None, logic_core, df)
    logger.info(f"Proceso Terminado. Items: {len(final_data)}. Tiempo: {time.time()-start_time:.2f}s")
    return final_data

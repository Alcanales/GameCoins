...
def enviar_correo_buylist(cli, items, clp, gc):
    ...
    for i in items:
        ...
        rows += f"""<tr style="background-color:{color}">
            ...
            <td style="padding:5px; font-weight:bold;">{i['razon']}</td>
            <td style="padding:5px; text-align:right;">${i.get('cash_clp',0):,}</td>
            <td style="padding:5px; text-align:right;">${i.get('gc_price',0):,}</td>
        </tr>"""

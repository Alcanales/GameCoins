...
from pydantic import BaseModel # CORRECCIÓN APLICADA AQUÍ
...
def inicializar_boveda():
    ...
    try:
        ...
        if updated:
            db.commit()
            logging.info("✅ Bóveda inicializada.")
...
# --- APP ---
app = FastAPI(title="GameQuest Points API", version="1.0.5-FINAL")
...
async def admin_analyze_csv(file: UploadFile = File(...), x_admin_user: str = Header(None), x_admin_pass: str = Header(None)):
...
def admin_config(req: ConfigRequest, db: Session = Depends(get_db), x_admin_user: str = Header(None), x_admin_pass: str = Header(None)):
    if x_admin_user != settings.ADMIN_USER or x_admin_pass != settings.ADMIN_PASS:
        raise HTTPException(status_code=401, detail="Acceso denegado")
    configs = {"JUMPSELLER_API_TOKEN": req.api_token, "JUMPSELLER_STORE": req.store_login, "JUMPSELLER_HOOKS_TOKEN": req.hooks_token}
...
@app.get("/admin/users")
def admin_get_users(db: Session = Depends(get_db), x_admin_user: str = Header(None), x_admin_pass: str = Header(None)):
    if x_admin_user != settings.ADMIN_USER or x_admin_pass != settings.ADMIN_PASS:
        raise HTTPException(status_code=401, detail="Acceso denegado")
    users = db.query(GameCoinUser).all()
    return [{"name": u.name if u.name else "Sin Nombre", "email": u.email, "saldo": u.saldo, "historico_canjeado": u.historico_canjeado} for u in users]

# --- SYNC JUMPSELLER ---
@app.post("/admin/sync_customers")
async def admin_sync_customers(db: Session = Depends(get_db), x_admin_user: str = Header(None), x_admin_pass: str = Header(None)):
    if x_admin_user != settings.ADMIN_USER or x_admin_pass != settings.ADMIN_PASS:
        raise HTTPException(status_code=401, detail="Acceso denegado")
...
def admin_adjust_balance(req: BalanceAdjustment, db: Session = Depends(get_db), x_admin_user: str = Header(None), x_admin_pass: str = Header(None)):
    if x_admin_user != settings.ADMIN_USER or x_admin_pass != settings.ADMIN_PASS:
        raise HTTPException(status_code=401, detail="Acceso denegado")
    ...
    if req.operation == "add": user.saldo += req.amount
    elif req.operation == "subtract": user.saldo = max(0, user.saldo - req.amount)
...
def send_offer_email(req: OfferRequest):
    try:
        ...
        for item in req.data[:50]:
            body += f"- {item.get('name')} | ${item.get('price_normal')}\n"
        if len(req.data) > 50: body += "\n...y mas."

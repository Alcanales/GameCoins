...
app = FastAPI(title="GameQuest API", version="Final-Fix")
...
def get_balance(email: str, db: Session = Depends(get_db)):
    # FIX: Convertimos a minúscula ANTES de buscar
    email_clean = email.lower().strip()
    
    # Buscamos ignorando mayúsculas/minúsculas en la DB también
...
def update_saldo(req: UpdateRequest, db: Session = Depends(get_db)):
    # FIX: Forzamos minúscula al guardar/actualizar
    email_clean = req.email.lower().strip()
    
    u = db.query(GameCoinUser).filter(func.lower(GameCoinUser.email) == email_clean).with_for_update().first()
    
    if not u:
        u = GameCoinUser(email=email_clean, rut="N/A", name="Usuario Manual", saldo=0)
        db.add(u)
...
def update_config(req: ConfigRequest, db: Session = Depends(get_db)):
    ...
    clean = req.store_login.lower().replace("https://", "").replace(".jumpseller.com", "").strip()
    set_cfg("JUMPSELLER_API_TOKEN", req.api_token.strip())
    set_cfg("JUMPSELLER_STORE", clean)
...
async def jumpseller_webhook(request: Request, x_jumpseller_hmac_sha256: str = Header(None), db: Session = Depends(get_db)):
    ...
    import base64
    if base64.b64encode(signature).decode() != x_jumpseller_hmac_sha256: raise HTTPException(401, "Firma inválida")
    try:
